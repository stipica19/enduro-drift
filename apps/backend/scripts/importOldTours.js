...
async function main() {
    ...
    for (const old of activeTours) {
        ...
        await newDb.collection("tourDates").updateOne({ number: Number(old.tour_number) }, {
            $set: {
    ...
    console.log(`Uvezeno/ažurirano ${imported} od ${oldTours.length} starih zapisa (${oldTours.length - imported} preskočeno, tour_number < 500 — završeni termini).`);
    await client.close();
}
main();
